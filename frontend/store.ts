...
export interface User {
  ...
  logoUrl?: string | null;
  plan?: 'free' | 'pro';
  /** Which paid plan is active (monthly/6month/yearly) */
  currentPlanKey?: 'free' | 'monthly' | '6month' | 'yearly';
...
export const useAppStore = create<AppState>()(
  ...
  fetchSeats: async () => {
    try {
      const list = await apiGet<Seat[]>(`/api/seats`);
      set({ seats: list });
    } catch {
      // keep local state
    }
  ...
  fetchSpaces: async () => {
    try {
      const list = await apiGet<Space[]>(`/api/spaces`);
      set({ spaces: list });
    } catch {
      // keep local state
    }
  ...
  createSpace: async (name) => {
    try {
      const space = await apiPost<Space>(`/api/spaces`, { name });
  ...
  fetchShifts: async () => {
    try {
      const list = await apiGet<Shift[]>(`/api/shifts`);
      set({ shifts: list });
    } catch {
      // keep local state
    }
  ...
  createShift: async (data) => {
    try {
      const shift = await apiPost<Shift>(`/api/shifts`, data);
  ...
  fetchAllocations: async (shiftId, spaceId) => {
    try {
      ...
      const list = await apiGet<SeatAllocation[]>(`/api/allocations`, params);
  ...
  assignAllocation: async (data) => {
    ...
    try {
      const created = await apiPost<SeatAllocation>(`/api/allocations`, data);
  ...
  cancelAllocation: async (allocationId) => {
    ...
    try {
      await apiPatch<SeatAllocation>(`/api/allocations/${allocationId}`, { status: 'cancelled' });
      return { ok: true };
    } catch (e) {
      set({ allocations: prev });
      const err = e as ApiError;
  ...
  bulkCreateSeats: async (totalSeats, spaceId) => {
    try {
      const list = await apiPost<Seat[]>(`/api/seats/bulk-create`, { totalSeats, spaceId: spaceId ?? null });
      set({ seats: list });
      return { ok: true };
    } catch (e) {
      const err = e as ApiError;
  ...
  updateSeatSpace: async (seatId, spaceId) => {
    ...
    try {
      const updated = await apiPatch<Seat>(`/api/seats/${seatId}`, { spaceId });
  ...
  assignSeat: async (seatId, studentId) => {
    ...
    try {
      ...
      const updated = await apiPost<Seat>(`/api/seats/assign`, { seatId, studentId });
      set((s) => ({ seats: s.seats.map((x) => (x.id === updated.id ? updated : x)) }));
      return { ok: true };
    } catch (e) {
      set({ seats: prev });
  ...
  unassignSeat: async (seatId) => {
    ...
    try {
      const updated = await apiPost<Seat>(`/api/seats/unassign`, { seatId });
      set((s) => ({ seats: s.seats.map((x) => (x.id === updated.id ? updated : x)) }));
      return { ok: true };
    } catch (e) {
      set({ seats: prev });
  ...
  login: async (usernameOrMobile, pinOrPassword, opts) => {
    try {
       ...
       */
      const data = await apiPost<{ user: User; authToken?: string; libraryCode?: string }>(`/api/auth/login`, {
        usernameOrMobile,
