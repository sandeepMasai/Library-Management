    ...
    return (
      <SafeAreaView style={styles.safe} edges={['left', 'right']}>
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 18 }}>
          {bootLoading ? (
            <Text style={{ color: '#94A3B8', fontWeight: '800' }}>Loading…</Text>
          ) : bootError ? (
            <>
              <Text style={{ color: '#DC2626', fontWeight: '900', marginBottom: 6 }}>Could not load</Text>
              <Text style={{ color: '#94A3B8', fontWeight: '700', textAlign: 'center' }}>{bootError}</Text>
              <TouchableOpacity
                onPress={logout}
                activeOpacity={0.85}
                style={{ marginTop: 14, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 14, backgroundColor: '#0F172A' }}
              >
