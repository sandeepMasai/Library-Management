...
export default function AdminDashboard() {
  ...
  const fetchDashboard = useCallback(async () => {
    const data = await apiGet<DashboardApiResponse>('/api/dashboard'); // fixed: missing quotes
    setDashboard(data);
  }, []);
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: headerTopBg }]} edges={['left', 'right']}>
      {/* Use global StatusBar handling from App.tsx (theme-driven). */}
      <ScrollView
        showsVerticalScrollIndicator={false}
        style={{ backgroundColor: theme.colors.background }}
        contentContainerStyle={[styles.scroll, { paddingBottom: scrollBottom + 20 }]}
      ...
      >
        ...
        <View style={styles.topPad}>
          <LinearGradient
            ...
            style={[styles.heroCard, { paddingTop: Math.max(20, 12 + Math.max(insets.top, 0)) }]}
          >
        ...
        <View style={styles.twoCol}>
          ...
          <AttendanceCard present={todayCount} absent={Math.max(0, total - todayCount)} />
        </View>
        ...
        {activityFeed.length === 0 ? (
          ...
          <View style={styles.activityCard}>
            {activityFeed.map((item, idx) => {
              let ago = '';
              try {
                ago = formatDistanceToNow(new Date(item.date), { addSuffix: true });
              } catch { }
...
// ── Sub-components ────────────────────────────────────────────────────────────
function KpiPill(props: { label: string; value: string; icon: keyof typeof Ionicons.glyphMap; tone: StatTone }) {
  const { mode } = useTheme();
  const styles = React.useMemo(() => makeStyles(mode), [mode]);
...
function QuickAction2(props: { icon: keyof typeof Ionicons.glyphMap; label: string; sub: string; tone: StatTone; onPress: () => void }) {
  const { mode } = useTheme();
  const styles = React.useMemo(() => makeStyles(mode), [mode]);
  const c = statTone(props.tone);
  return (
    <TouchableOpacity onPress={props.onPress} activeOpacity={0.86} style={styles.qa2}>
      ...
      <View style={{ flex: 1, minWidth: 0 }}>
        <Text style={styles.qa2Label} numberOfLines={1}>
          {props.label}
        </Text>
        <Text style={styles.qa2Sub} numberOfLines={1}>
          {props.sub}
        </Text>
      </View>
...
function makeStyles(mode: 'light' | 'dark') {
  ...
  return StyleSheet.create({
    ...
    topPad: { paddingHorizontal: 16, paddingTop: 20 },
    heroCard: {
      borderRadius: 22,
      padding: 16,
      overflow: 'hidden',
    ...
    heroKpis: { marginTop: 14, flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
    kpiPill: {
      flexGrow: 1,
      minWidth: 140,
      flexDirection: 'row',
    ...
    twoCol: { flexDirection: 'row', gap: 12, paddingHorizontal: 16, marginTop: 12, marginBottom: 6 },
    card: { flex: 1, backgroundColor: theme.colors.surface, borderRadius: 20, borderWidth: 1, borderColor: theme.colors.border, padding: 14 },
    ...
    payMini: { backgroundColor: theme.colors.background, borderRadius: 16, borderWidth: 1, borderColor: theme.colors.border, paddingVertical: 10, paddingHorizontal: 12 },
    ...
    actionsWrap: { paddingHorizontal: 16, gap: 10, marginBottom: 6 },
    qa2: { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: theme.colors.surface, borderRadius: 18, borderWidth: 1, borderColor: theme.colors.border, paddingHorizontal: 14, paddingVertical: 12 },
    ...
    // ── Alerts ──
    alertsWrap: { flexDirection: 'row', flexWrap: 'wrap', paddingHorizontal: 16, gap: 8, marginBottom: 6 },
    alertPill: { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 20 },
    ...
    timelineLine: { position: 'absolute', left: 19, top: 48, bottom: 0, width: 1.5, backgroundColor: theme.colors.border },
    ...
    activityInitialBox: { width: 40, height: 40, borderRadius: 20, alignItems: 'center', justifyContent: 'center' },
    ...
    activityIconBox: { width: 40, height: 40, borderRadius: 12, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
    ...
    // ── Empty ──
    emptyActivity: { marginHorizontal: 16, backgroundColor: theme.colors.surface, borderRadius: 18, alignItems: 'center', paddingVertical: 32, borderWidth: 1, borderColor: theme.colors.border },
