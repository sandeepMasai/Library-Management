...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    if (loginType === 'student') {
      ...
      // Admin
      if (!id || !sec) {
        setInfoModal({ title: 'Required', description: 'Please enter username and password.' });
        return;
      }
    }
    ...
    if (!result.ok) {
      setInfoModal({
        ...
        description: result.message || 'Invalid credentials or account blocked.',
      });
    }
  };
...
function makeStyles() {
  ...
  apiHint: { marginTop: -10, marginBottom: 14, textAlign: 'center', color: theme.colors.mutedText, fontWeight: '700', fontSize: 12 },
