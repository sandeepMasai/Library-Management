...
type PlanKey = 'free' | 'monthly' | '6month' | 'yearly';
type Plan = { key: PlanKey; title: string; price: number; sub: string };
...
export default function PlanSelectionScreen(props: { embedded?: boolean } = {}) {
  ...
  const plans: Plan[] = useMemo(
    () => [
      { key: 'free', title: 'Free Plan', price: 0, sub: 'Basic access · Limited features' },
      { key: 'monthly', title: 'Pro Monthly', price: 999, sub: 'Pay monthly · Cancel anytime' },
      { key: '6month', title: 'Pro 6 Month', price: 4999, sub: 'Best for regular libraries' },
      { key: 'yearly', title: 'Pro Yearly', price: 9999, sub: 'Save more · Pro benefits' },
    ],
    []
  );
  ...
  const startPayment = useCallback(
    async (plan: Plan) => {
      ...
      if (plan.key === 'free') {
        setBusy(true);
        try {
          const upgrade = await apiPost<{ ok: boolean; user?: any; message?: string }>(`/api/subscription/upgrade`, {
            planKey: 'free_trial',
          });
          if (upgrade?.user) {
            useAppStore.setState({ currentUser: upgrade.user });
          }
          Alert.alert('Activated', 'Free plan activated.');
          navigation.goBack();
        } catch (e: any) {
          const err = e as ApiError;
          const msg = err?.message || 'Please try again.';
          const code = typeof (err as any)?.status === 'number' ? ` (HTTP ${(err as any).status})` : '';
          Alert.alert('Activation failed', `${msg}${code}`);
        } finally {
          setBusy(false);
        }
        return;
      ...
      try {
        const order = await apiPost<{
          ...
          currency: 'INR';
          plan: PlanKey;
        }>(`/api/payment/create-order`, { plan: plan.key, amount: plan.price });
  ...
  return (
    <SafeAreaView style={[styles.safe, embedded && styles.safeEmbedded]} edges={embedded ? [] : ['left', 'right', 'bottom']}>
      <ScrollView style={[styles.root, embedded && styles.rootEmbedded]} contentContainerStyle={styles.content}>
              ...
              >
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 10 }}>
                  ...
                  <View style={{ alignItems: 'flex-end' }}>
                    <Text style={styles.price}>{p.key === 'free' ? 'FREE' : `₹${p.price}`}</Text>
                    <Text style={styles.priceSub}>
                      {p.key === 'free' ? '' : p.key === 'monthly' ? '/month' : p.key === '6month' ? '/6 months' : '/year'}
                    </Text>
                  </View>
                </View>
              </TouchableOpacity>
        ...
        >
          <Ionicons
            name={picked?.key === 'free' ? 'checkmark-circle-outline' : 'lock-closed-outline'}
            size={18}
