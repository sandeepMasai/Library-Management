...
type PlanRow = {
  id: string;
  key: 'free' | 'monthly' | '6month' | 'yearly';
  name: string;
  price: number;
  discount: number;
  finalPrice: number;
...
export default function SubscriptionScreen() {
  ...
  // Backend should provide currentPlanKey (monthly/6month/yearly/free). Keep a safe fallback so UI never loses tick.
  ...
  // - and currentPlanKey indicates a paid tier (monthly/6month/yearly)
  const paidPlanKey =
    currentPlanKeyRaw === 'monthly' || currentPlanKeyRaw === '6month' || currentPlanKeyRaw === 'yearly'
      ? currentPlanKeyRaw
      : null;
  ...
  const effectivePaidPlanKey =
    paidPlanKey || (plan === 'pro' && isActiveUntilExpiry && subStatus !== 'expired' ? 'monthly' : null);
  const hasActiveProAccess = Boolean(effectivePaidPlanKey && isActiveUntilExpiry && subStatus !== 'expired');
  const hasActiveFreeTrial =
    plan === 'free' && isActiveUntilExpiry && subStatus !== 'expired' && (currentPlanKeyRaw ?? 'free') === 'free';
  ...
  const activePlanKey = hasActiveProAccess ? (currentPlanKey ?? null) : hasActiveFreeTrial ? 'free' : null;
  const isExpired = subStatus === 'expired' || (plan === 'free' && !hasActiveFreeTrial);
  const canCancel =
    !isExpired && Boolean(expiryDate) && (hasActiveProAccess || plan === 'pro' || subStatus === 'cancelled');
  ...
  const visiblePlans = useMemo(() => {
    return plans.filter((p) => {
      if (p.key !== 'free') return true;
      // Show free plan only while trial is currently active.
      return hasActiveFreeTrial;
    });
  }, [plans, hasActiveFreeTrial]);
  ...
  const startPayment = useCallback(
    async (planRow: PlanRow) => {
      ...
      // Free plan is a one-time trial that is auto-assigned on registration.
      // Do not "activate" it again from UI.
      if (planRow.key === 'free') {
        Alert.alert('Free trial', hasActiveFreeTrial ? 'Free trial is already active.' : 'Free trial is one-time only.');
        return;
      }
      ...
      <View style={styles.currentCard}>
        ...
        <View style={styles.currentMetaRow}>
          ...
          <View style={styles.metaItem}>
            ...
            <Text style={styles.metaVal}>{planLabel(activePlanKey)}</Text>
          </View>
        </View>
      </View>
      ...
      {planChangeLocked ? (
        <View style={styles.lockInfo}>
          ...
          <Text style={styles.lockInfoTxt}>
            Your plan is active till {expiryDate ? new Date(expiryDate).toLocaleDateString() : 'expiry'}. You can change plan after it ends.
          </Text>
        </View>
      ) : null}
      <View style={{ gap: subSpacing.gap }}>
        {visiblePlans.map((p) => {
          ...
          const isTrialTick = Boolean(hasActiveFreeTrial && p.key === 'free');
          ...
          return (
            <TouchableOpacity
              ...
              disabled={payBusy || planChangeLocked || p.key === 'free'}
            >
              ...
              <View style={{ alignItems: 'flex-end', paddingLeft: 10 }}>
                {p.key === 'free' ? <Text style={styles.freeTxt}>FREE</Text> : null}
                {isPaidTick ? (
