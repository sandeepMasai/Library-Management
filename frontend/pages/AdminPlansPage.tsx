...
type PlanRow = {
  _id: string;
  name: string;
  key: 'free' | 'monthly' | '6month' | 'yearly' | string;
  price: number;
  discount: number;
  finalPrice: number;
  duration: number;
...
export default function AdminPlansPage() {
  ...
  const save = useCallback(async () => {
    ...
    const discount = Number(fDiscount);
    const duration = Number(fDuration);
    ...
    if (!Number.isFinite(price) || price < 0) return Alert.alert('Invalid', 'Price must be a number ≥ 0');
    if (!Number.isFinite(discount) || discount < 0 || discount > 100) return Alert.alert('Invalid', 'Discount must be 0–100');
    if (!Number.isFinite(duration) || duration < 0) return Alert.alert('Invalid', 'Duration must be ≥ 0 days');
              ...
              return (
                <View key={p._id} style={styles.card}>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
                    ...
                    <View style={{ alignItems: 'flex-end' }}>
                      <View style={{ flexDirection: 'row', alignItems: 'baseline', gap: 8 }}>
                        ...
                        <Text style={styles.price}>₹{hasDiscount ? p.finalPrice : p.price}</Text>
                      </View>
                  ...
                  <View style={styles.actionsRow}>
                    ...
                    <TouchableOpacity onPress={() => remove(p)} style={[styles.actionBtn, { borderColor: theme.colors.danger }]} activeOpacity={0.9}>
                      <Ionicons name="trash-outline" size={16} color={theme.colors.danger} />
                      <Text style={[styles.actionTxt, { color: theme.colors.danger }]}>Delete</Text>
                    </TouchableOpacity>
                  </View>
                </View>
              );
...
const styles = StyleSheet.create({
  ...
  discountTxt: { marginTop: 4, color: '#059669', fontWeight: '900', fontSize: 12 },
