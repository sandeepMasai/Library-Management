  ...
  return (
    <View style={styles.paymentRow}>
      ...
      <View style={{ flex: 1, minWidth: 0 }}>
        ...
        <Text style={styles.paymentMeta} numberOfLines={1}>{`Date: ${formatDate(row.date)}`}</Text>
      </View>
...
<html>
  ...
  <body>
    <div class="card">
      ...
      <div class="row"><div><div class="k">Date</div><div class="v">${formatDate(p.date)}</div></div></div>
    </div>
  </body>
</html>`;
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['left', 'right', 'bottom']}>
      ...
      <ScrollView style={styles.root} contentContainerStyle={{ padding: theme.spacing.lg, paddingBottom: theme.spacing.xl }}>
        ...
        {/* Subscription period */}
        <PeriodCard subscription={detail.subscription} progress={period.progress} daysLeft={period.daysLeft} />
...
function PlanCard({
  ...
  const per = subscription.plan === 'monthly' ? '/ month' : subscription.plan === 'free' ? '' : '';
    ...
    <Card title="Subscription Period">
      <View style={styles.periodRow}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.v}>{formatDate(subscription.startDate)}</Text>
        </View>
        <View style={{ width: 12 }} />
        <View style={{ flex: 1, alignItems: 'flex-end' }}>
          <Text style={styles.k}>Expiry Date</Text>
          <Text style={styles.v}>{formatDate(subscription.expiryDate)}</Text>
        </View>
      </View>

      <View style={{ marginTop: 12 }}>
        <ProgressBar progress={progress} />
        <Text style={styles.daysLeftTxt}>
