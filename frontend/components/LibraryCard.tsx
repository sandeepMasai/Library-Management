...
export default function LibraryCard({
  ...
  const libraryName = (user as any)?.library?.libraryName ? String((user as any).library.libraryName) : '';
  const displayName = titleCase(user.name);
  const displayLibrary = libraryName ? libraryName.toUpperCase() : '';
...
const styles = StyleSheet.create({
  ...
  libraryName: { marginTop: 2, fontSize: 12, fontWeight: '800', color: 'rgba(255,255,255,0.82)' },
