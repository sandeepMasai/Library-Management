 ...
 */
router.get("/payment-plans", requireAdminAuth, async (_req, res) => {
  try {
 ...
 * - active?: boolean
 */
router.put("/payment-plans/:key", requireAdminAuth, async (req, res) => {
  try {
    ...
    invalidatePlanConfigCache("admin_payment_plan_update");
 ...
 */
router.get("/library/:id", requireAdminAuth, async (req, res) => {
  try {
    ...
    return res.json({
      ...
      library: {
        ...
        isActive: Boolean(lib.isActive),
        plan: lib.plan,
        planStartDate: libPeriod.startDate,
        planExpiryDate: libPeriod.expiryDate,
        subscriptionStatus: lib.subscriptionStatus || "inactive",
        cancelledAt: lib.cancelledAt?.toISOString?.() || null,
        cancelReason: lib.cancelReason || null,
        cancelNote: lib.cancelNote || null,
 ...
 */
router.get("/library/:id/subscription", requireAdminAuth, async (req, res) => {
  try {
    const id = String(req.params.id || "").trim();
    if (!mongoose.Types.ObjectId.isValid(id)) return res.status(400).json({ message: "Invalid library id" });
    ...
    const plan = latestSub?.plan || (lib.plan === "pro" ? "monthly" : "none");
    const price = typeof latestSub?.price === "number" ? latestSub.price : lib.plan === "pro" ? 999 : 0;
    const { startDate, expiryDate } = resolveLibrarySubscriptionPeriod(lib, latestSub);
 ...
 * Body: { libraryId }
 */
router.post("/subscription/cancel", requireAdminAuth, async (req, res) => {
  try {
 ...
 */
router.patch("/libraries/:id/block", requireAdminAuth, async (req, res) => {
  try {
    const id = String(req.params.id || "").trim();
    if (!mongoose.Types.ObjectId.isValid(id)) return res.status(400).json({ message: "Invalid library id" });
 ...
 */
router.delete("/libraries/:id", requireAdminAuth, async (req, res) => {
  ...
  try {
    ...
    // Rule (Super Admin): allow delete ONLY when PRO is active until expiry.
    const expiryMs = lib.planExpiryDate ? new Date(lib.planExpiryDate).getTime() : null;
    const planActiveUntilExpiry =
      lib.plan === "pro" &&
      lib.subscriptionStatus !== "expired" &&
      expiryMs &&
      Number.isFinite(expiryMs) &&
      Date.now() < expiryMs;
    if (!planActiveUntilExpiry) {
      await session.abortTransaction();
      session.endSession();
      return res.status(400).json({
        message: "Only PRO active libraries can be deleted by Super Admin.",
        code: "DELETE_ONLY_PRO_ACTIVE",
        planExpiryDate: lib.planExpiryDate?.toISOString?.() || null,
      });
    }
 ...
 *
 * Logic:
 * - Libraries without an active paid term appear as plan `none`.
 */
router.get("/subscriptions", requireAdminAuth, async (req, res) => {
  try {
    ...
    function fallbackPlanMeta(lib) {
      if (lib.plan !== "pro") return { plan: "none", price: 0 };
    ...
    const rows = list
      .map((lib) => {
        ...
        const { startDate, expiryDate } = resolveLibrarySubscriptionPeriod(lib, sub);
        ...
        const pay = sub?.paymentStatus || "paid";
 ...
 */
router.post("/notify", requireAdminAuth, async (req, res) => {
  try {
 ...
 *
 * Returns recently sent admin notifications (de-duplicated):
 * - When target === "all", backend stores one Notification per library.
 *   This endpoint groups those into a single row for UI.
 * - When a single library is targeted, returns that libraryId as `target`.
 *
 * Response:
 * { ok: true, notifications: [{ id, title, message, target, createdAt, count }] }
 */
router.get("/notifications", requireAdminAuth, async (req, res) => {
  try {
    ...
    const rows = await Notification.aggregate([
      ...
      {
        $group: {
          _id: { title: "$title", message: "$message", date: "$date" },
          anyId: { $first: "$_id" },
          title: { $first: "$title" },
          message: { $first: "$message" },
          date: { $first: "$date" },
          libraries: { $addToSet: "$libraryId" },
          count: { $sum: 1 },
        },
      },
      { $sort: { date: -1 } },
      { $limit: limit },
      {
        $project: {
          ...
          title: 1,
          message: 1,
          createdAt: { $dateToString: { date: "$date" } },
          count: 1,
          target: {
            $cond: [{ $gt: ["$count", 1] }, "all", { $toString: { $first: "$libraries" } }],
          },
        },
      },
    ]);
 ...
 *
 * Updates a recently sent notification.
 * - If the notification was broadcast ("all"), update all docs in that broadcast group.
 * - Otherwise update the single doc.
 *
 * Body: { title, message }
 */
router.put("/notifications/:id", requireAdminAuth, async (req, res) => {
  try {
    const id = String(req.params.id || "").trim();
 ...
 *
 * Deletes a recently sent notification.
 * - If the notification was broadcast ("all"), delete all docs in that broadcast group.
 * - Otherwise delete the single doc.
 */
