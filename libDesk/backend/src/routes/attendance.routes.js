...
async function repairAttendanceIndexes() {
  try {
    ...
    logger.warn("Attendance route index repair skipped", {
      name: error?.name,
      code: error?.code,
  ...
  const safeDate =
    attendance?.attendanceDate instanceof Date
      ...
      : attendance?.attendanceDate
        ? new Date(attendance.attendanceDate).toISOString()
...
function getUtcDayRange(value = new Date()) {
...
router.post("/token", requireAuth, requireRole("admin", "library"), requireNotExpiredSubscription, async (req, res) => {
  try {
...
router.get("/", requireAuth, requireRole("admin", "library"), requireNotExpiredSubscription, async (req, res) => {
  try {
    ...
    const range = getUtcDayRange(dateQuery);
    if (!range) {
...
router.get("/today", requireAuth, requireRole("admin", "library"), requireNotExpiredSubscription, async (req, res) => {
  try {
    // Multi-tenant security enforced
    const libraryId = requireLibraryIdForAdmin(req, res);
    if (!libraryId) return;
...
router.get("/student/:studentId", requireAuth, requireRole("admin", "library", "student"), requireNotExpiredSubscription, async (req, res) => {
  try {
    ...
    const monthRange = getUtcMonthRange(year, month);
    ...
    for (const item of list) {
      const dateKey = toDateKey(new Date(item.attendanceDate));
      if (seen.has(dateKey)) continue;
      seen.add(dateKey);
      result.push({ date: dateKey, status: item.status || "present" });
    }
...
router.post("/mark", async (req, res) => {
  try {
    ...
    let authPayload;
    try {
      authPayload = verifyAccessToken(bearer);
    } catch {
    ...
    const now = new Date();
    const todayDate = toUtcStartOfDay(now);
    ...
    if (error?.code === 11000) {
      ...
      let libraryId = "";
      try {
        const authPayload = bearer ? verifyAccessToken(bearer) : null;
