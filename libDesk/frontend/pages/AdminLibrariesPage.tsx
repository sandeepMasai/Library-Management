...
export default function AdminLibrariesPage() {
  ...
  const onDelete = async (lib: LibraryRow) => {
    ...
    if (!planActiveUntilExpiry) {
      Alert.alert('Not allowed', 'Only PRO active libraries can be deleted by Super Admin.');
      return;
    }
    ...
    Alert.alert('Delete', `Delete "${lib.name}"?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          const prev = rows;
          const wasLastRowOnPage = prev.length === 1;
          setRows((list) => list.filter((x) => x.id !== lib.id));
          try {
            await apiDelete(`/api/admin/libraries/${lib.id}`);
            // Keep pagination consistent and avoid showing stale data.
            const nextPage = wasLastRowOnPage && page > 1 ? page - 1 : page;
            ...
            <View style={styles.card}>
              ...
              <View style={styles.row}>
                ...
                <View style={{ flexDirection: 'row', gap: 8, marginLeft: 'auto' }}>
                  ...
                  {(() => {
                    const expiryMs = item.planExpiryDate ? new Date(item.planExpiryDate).getTime() : null;
                    const planActiveUntilExpiry =
                      item.plan === 'pro' &&
                      ...
                      Number.isFinite(expiryMs) &&
                      Date.now() < expiryMs;
                    if (!planActiveUntilExpiry) return null;
                    return (
                      <TouchableOpacity
                        onPress={() => onDelete(item)}
                        style={[styles.smallBtn, { backgroundColor: '#FEF2F2', borderColor: '#FECACA' }]}
                        activeOpacity={0.85}
                      >
...
function makeStyles(_mode: 'light' | 'dark') {
  return StyleSheet.create({
    ...
    primaryBtn: { marginTop: 14, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 14, backgroundColor: theme.colors.dark },
    primaryTxt: { color: theme.colors.surface, fontWeight: '900' },
  });
}
