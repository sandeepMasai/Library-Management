  ...
  const handleBarCodeScanned = async ({ data }: { type: string; data: string }) => {
    ...
    if (result.ok && !result.alreadyMarked) {
      ...
      setResultModal({
        ...
        description: result.message || 'Attendance already recorded today',
        confirmText: 'OK',
        afterOk: () => {
          setScanned(false);
          navigation.navigate('Home');
      ...
      setResultModal({
        tone: 'danger',
        title: 'Error',
        description: result.message || 'Invalid QR Code.',
        confirmText: 'Try Again',
        afterOk: () => setScanned(false),
      });
    }
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top, paddingBottom: insets.bottom + 8 }]}>
      ...
      <ConfirmModal
        ...
        title={resultModal?.title ?? 'Info'}
        description={resultModal?.description}
        showCancel={false}
        confirmText={resultModal?.confirmText ?? 'OK'}
        confirmIcon={resultModal?.tone === 'danger' ? 'warning' : 'checkmark-outline'}
        onCancel={() => {
          const fn = resultModal?.afterOk;
          setResultModal(null);
          fn?.();
        }}
        onConfirm={() => {
