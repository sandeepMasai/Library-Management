...
type PlanKey = 'pro_monthly' | 'pro_6_month' | 'pro_yearly';

function formatPlanExpiryLine(d: string | null | undefined) {
  if (!d) return '—';
  const t = new Date(d);
...
export default function LibrarySettingsScreen() {
  ...
  const planInfo = useMemo(() => {
    ...
    return {
      plan: (currentUser.plan || 'none').toUpperCase(),
      expiry: formatPlanExpiryLine(currentUser.planExpiryDate ?? null),
    };
  }, [currentUser]);
  ...
  const onUpgrade = async (key: PlanKey) => {
    setBusy(key);
    // Manual payment-based plans: take user to the Subscription screen.
