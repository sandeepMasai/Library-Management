...
export default function AdminStudentForm() {
  ...
  const seatPickerSeats = useMemo<Seat[]>(() => {
    return seats.slice().sort((a, b) => a.number - b.number);
  }, [seats]);

  const selectedShift = useMemo(() => shifts.find((s) => s.id === allocShiftId) ?? null, [allocShiftId, shifts]);
  ...
  const seatPickerColumns = width >= 430 ? 4 : 3;
  const seatPickerWidth = Math.min(width - 36, 520);
  const seatPickerGap = 10;
  const seatPickerCardWidth = Math.floor((seatPickerWidth - 28 - seatPickerGap * (seatPickerColumns - 1)) / seatPickerColumns);
              ...
              <>
                ...
                {allocEnabled ? (
                  <>
                    ...
                    <TouchableOpacity
                      activeOpacity={0.9}
                      style={[styles.seatPickBtn, selectedSeatNumber && styles.seatPickBtnOn]}
                      onPress={() => {
                        if (!allocShiftId) {
                    ...
                    >
                      <View style={styles.seatPickIcon}>
                        <Ionicons name="apps-outline" size={18} color={selectedSeatNumber ? theme.colors.primary : theme.colors.mutedText} />
                      </View>
                      <Text style={[styles.seatPickTxt, selectedSeatNumber && styles.seatPickTxtOn]}>
                      ...
                      <View style={styles.modalBackdrop}>
                        <View style={styles.modalCard}>
                          <View style={styles.modalHead}>
                            <View>
                              <Text style={styles.modalKicker}>SEAT MAP</Text>
                              <Text style={styles.modalTitle}>Select seat number</Text>
                              <Text style={styles.modalSub}>
                                {selectedShift ? `${selectedShift.name} shift` : 'Choose shift first'} · {vacantSeats.length} available
                              </Text>
                            </View>
                            <TouchableOpacity style={styles.modalCloseBtn} onPress={() => setSeatPickerOpen(false)} hitSlop={12}>
                          ...
                          <View style={styles.seatLegendRow}>
                            <View style={styles.seatLegendItem}>
                              <View style={[styles.seatLegendDot, { backgroundColor: theme.colors.success }]} />
                              <Text style={styles.seatLegendTxt}>Available</Text>
                            </View>
                            <View style={styles.seatLegendItem}>
                              <View style={[styles.seatLegendDot, { backgroundColor: theme.colors.primary }]} />
                              <Text style={styles.seatLegendTxt}>Selected</Text>
                            </View>
                            <View style={styles.seatLegendItem}>
                              <View style={[styles.seatLegendDot, { backgroundColor: theme.colors.danger }]} />
                              <Text style={styles.seatLegendTxt}>Booked</Text>
                            </View>
                          </View>

                          <FlatList
                            data={seatPickerSeats}
                            keyExtractor={(x: Seat) => x.id}
                            key={`student-seat-picker-${seatPickerColumns}`}
                            numColumns={seatPickerColumns}
                            columnWrapperStyle={styles.seatMiniRow}
                            contentContainerStyle={styles.seatMiniList}
                            showsVerticalScrollIndicator={false}
                            renderItem={({ item }: { item: Seat }) => {
                              const selected = allocSeatId === item.id;
                              const booked = occupiedSeatIds.has(item.id);
                              return (
                                <TouchableOpacity
                                  activeOpacity={booked ? 1 : 0.78}
                                  disabled={booked}
                                  style={[
                                    styles.seatMini,
                                    { width: seatPickerCardWidth },
                                    selected && styles.seatMiniOn,
                                    booked && styles.seatMiniBooked,
                                  ]}
                                  onPress={() => {
                                ...
                                >
                                  <View style={[styles.seatMiniIcon, selected && styles.seatMiniIconOn, booked && styles.seatMiniIconBooked]}>
                                    <Ionicons
                                      name={booked ? 'lock-closed' : selected ? 'checkmark' : 'add'}
                                      size={13}
                                      color={booked ? theme.colors.danger : selected ? theme.colors.surface : theme.colors.primary}
                                    />
                                  </View>
                                  <Text style={[styles.seatMiniTxt, selected && styles.seatMiniTxtOn, booked && styles.seatMiniTxtBooked]}>
                                    {item.number}
                                  </Text>
                                  <Text style={[styles.seatMiniStatus, selected && styles.seatMiniStatusOn, booked && styles.seatMiniStatusBooked]}>
                            ...
                            ListEmptyComponent={
                              <View style={styles.emptySeatPicker}>
                                <Ionicons name="albums-outline" size={34} color={theme.colors.mutedText} />
                                <Text style={styles.emptySeatTitle}>No seats found</Text>
                                <Text style={styles.emptySeatSub}>Create seats first, then assign one to this student.</Text>
                              </View>
                            }
                          />
                        </View>
...
function makeStyles(mode: 'light' | 'dark') {
  ...
  seatPickTxtOn: { color: theme.colors.primary },
  seatMiniRow: { gap: 10 },
  seatMiniList: { paddingTop: 12, paddingBottom: 6 },
  seatMini: {
    minHeight: 84,
    borderRadius: 16,
    backgroundColor: theme.colors.background,
  ...
  seatMiniIconOn: {
    backgroundColor: withAlpha(theme.colors.surface, 0.22),
    borderColor: withAlpha(theme.colors.surface, 0.32),
  },
  seatMiniIconBooked: {
    backgroundColor: withAlpha(theme.colors.danger, 0.12),
    borderColor: withAlpha(theme.colors.danger, 0.20),
  },
  seatMiniTxt: { fontSize: 16, fontWeight: '900', color: theme.colors.text },
  seatMiniTxtOn: { color: theme.colors.surface },
  seatMiniTxtBooked: { color: theme.colors.danger },
  seatMiniStatus: { marginTop: 2, fontSize: 9, fontWeight: '900', color: theme.colors.mutedText },
  seatMiniStatusOn: { color: theme.colors.surface },
  seatMiniStatusBooked: { color: theme.colors.danger },
  ...
  modalKicker: { fontSize: 10, fontWeight: '900', color: theme.colors.primary, letterSpacing: 1.2 },
  modalTitle: { marginTop: 2, fontSize: 20, fontWeight: '900', color: theme.colors.text, letterSpacing: -0.4 },
  modalSub: { marginTop: 4, fontSize: 12, fontWeight: '800', color: theme.colors.mutedText },
  modalCloseBtn: {
    width: 36,
    height: 36,
    borderRadius: 12,
  ...
  seatLegendDot: { width: 9, height: 9, borderRadius: 5 },
  seatLegendTxt: { fontSize: 10, fontWeight: '900', color: theme.colors.mutedText },
  emptySeatPicker: { paddingVertical: 26, alignItems: 'center', paddingHorizontal: 18 },
  emptySeatTitle: { marginTop: 10, fontSize: 15, fontWeight: '900', color: theme.colors.text },
  emptySeatSub: { marginTop: 4, fontSize: 12, fontWeight: '700', color: theme.colors.mutedText, textAlign: 'center' },
  saveBtn: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
