...
export default function StudentCard({ student, onEdit, onBlock, onDelete }: StudentCardProps) {
  ...
  const editBg = isDark ? 'rgba(99,102,241,0.18)' : '#EEF2FF';
  const editFg = theme.colors.primary;
  const blockFg = student.isBlocked ? theme.colors.success : theme.colors.danger;
  const blockBg = student.isBlocked ? (isDark ? 'rgba(34,197,94,0.18)' : '#DCFCE7') : (isDark ? 'rgba(239,68,68,0.16)' : '#FEF2F2');
  const deleteBg = isDark ? 'rgba(239,68,68,0.16)' : '#FEF2F2';
  ...
  return (
    <TouchableOpacity style={styles.card} onPress={onEdit} activeOpacity={0.85}>
      ...
      <View style={styles.actions}>
        <ActionBtn icon="create-outline" label="Edit" color={editFg} bg={editBg} onPress={onEdit} />
        <ActionBtn
          ...
          onPress={onBlock}
        />
        <ActionBtn icon="trash-outline" label="Delete" color={deleteFg} bg={deleteBg} onPress={onDelete} />
      </View>
    </TouchableOpacity>
  );
}
